# The_Book/src/adapters/hebrew_transcription_adapter.py
# Generic adapter for Genesis Hebrew manuscript transcription payloads.
...
class HebrewGenesisTranscriptionAdapter(BaseAdapter):
    """Fetch and parse Genesis Hebrew transcription data into WitnessRecords."""
    ...
    async def fetch(
        ...
        if payload is None:
            ...
            if not endpoint:
                raise ValueError(
                    "Hebrew transcription adapter requires api_endpoint in config/sources.yaml "
                    "or payload_override"
                )
    ...
    async def _load_payload(endpoint: str, timeout_seconds: int) -> Dict[str, Any]:
        if endpoint.startswith("http://") or endpoint.startswith("https://"):
            ...
            if not isinstance(payload, dict):
                raise ValueError("Remote transcription payload must be a JSON object")
            return payload

        payload_path = Path(endpoint)
        if not payload_path.exists():
            raise FileNotFoundError(f"Transcription payload file not found: {payload_path}")

        with payload_path.open("r", encoding="utf-8-sig") as handle:
            payload = json.load(handle)

        if not isinstance(payload, dict):
            raise ValueError("Local transcription payload must be a JSON object")
    ...
    def _extract_chapter_verses(payload: Dict[str, Any], chapter: int) -> List[Dict[str, Any]]:
        ...
        if not isinstance(verses, list):
            raise ValueError("Transcription payload must include a 'verses' array")

        source_uri_base = str(payload.get("source_uri") or payload.get("source_url") or "").strip()
        if not source_uri_base:
            source_uri_base = "hebrew-manuscript://genesis/transcription"

        source_version_date = HebrewGenesisTranscriptionAdapter._coerce_datetime(
        ...
        for verse in verses:
            ...
            verse_chapter = HebrewGenesisTranscriptionAdapter._parse_int(verse.get("chapter"), default=chapter)
            if verse_chapter != chapter:
                continue

            verse_number = HebrewGenesisTranscriptionAdapter._parse_int(verse.get("verse"))
            if verse_number is None:
                continue

            verse_text = HebrewGenesisTranscriptionAdapter._normalize_text(
            ...
            note_parts = [f"Genesis {chapter}:{verse_number}", f"Manuscript siglum: {manuscript_siglum}"]
        ...
        if not extracted:
            raise ValueError(f"No transcription verses found for Genesis chapter {chapter}")
