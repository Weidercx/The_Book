#!/usr/bin/env python3
"""Pull Genesis chapter 1 from OSHB and write verse-level JSONL output."""
...
def load_oshb_config(config_path: Path) -> Dict[str, Any]:
    ...
    oshb_config = archives.get("oshb")
    if not oshb_config:
        raise ValueError("Missing 'oshb' configuration in config/sources.yaml")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Pull Genesis chapter from OSHB into verse-level WitnessRecords"
    )
    parser.add_argument(
        "--chapter",
        type=int,
